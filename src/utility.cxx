...
char ipr::util::string::operator[](int i) const {
   if (i < 0 || i >= length)
...
ipr::util::string* ipr::util::string::arena::allocate(int n) {
   ...
   // in a fresh pool object, just allocate that string on its own
      ...
      pool* new_pool = static_cast<pool*>(operator new(poolsz + (n - bufsz)));
...
const ipr::util::string* ipr::util::string::arena::make_string(const char* s,
                                                               int n) {
