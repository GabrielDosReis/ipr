...
namespace ipr {
   namespace impl {
      ...
      Token::Token(const ipr::String& s, const Source_location& l, TokenValue v,
                   TokenCategory c)
      ...
      master_decl_data<ipr::Named_map>::master_decl_data(impl::Overload* ovl,
                                                         const ipr::Type& t)
            : Base(this), overload_entry(t), def(0), langlinkage(0), primary(0),
              home(0), overload(ovl) {}
      ...
      int decl_sequence::size() const { return decls.size(); }

      const ipr::Decl& decl_sequence::get(int i) const {
         scope_datum* result = decls.find(i, scope_datum::comp());
         return *util::check(result)->decl;
      }

      void decl_sequence::insert(scope_datum* s) {
      ...
      Overload::Overload(const ipr::Name& n) : name(n), where(0) {}

      int Overload::size() const { return entries.size(); }

      const ipr::Decl& Overload::get(int i) const {
      ...
      impl::overload_entry* Overload::lookup(const ipr::Type& t) const {
         return entries.find(t, node_compare());
      }

      template<class T>
      void Overload::push_back(master_decl_data<T>* data) {
      ...
      singleton_overload::singleton_overload(const ipr::Decl& d) : seq(d) {}

      const ipr::Type& singleton_overload::type() const {
         return seq.datum.type();
      }

      int singleton_overload::size() const { return 1; }

      const ipr::Decl& singleton_overload::get(int i) const {
         if (i != 1)
      ...
      const ipr::Type& empty_overload::type() const {
         throw std::domain_error("empty_overload::type");
      }

      int empty_overload::size() const { return 0; }

      const ipr::Decl& empty_overload::get(int) const {
      ...
      // -- impl::Rname --
      // -----------------
      Rname::Rname(Rep r) : impl::Ternary<impl::Node<ipr::Rname>>(r) {}

      const ipr::Type& Rname::type() const { return rep.first; }
      ...
      Optional<ipr::Expr> Alias::initializer() const { return { aliasee }; }

      const ipr::Region& Alias::lexical_region() const {
      ...
      const ipr::Expr& Bitfield::precision() const {
         return *util::check(length);
      }

      const ipr::Udt<ipr::Decl>& Bitfield::membership() const {
         return *util::check(member_of);
      }

      Optional<ipr::Expr> Bitfield::initializer() const { return { init }; }

      const ipr::Region& Bitfield::lexical_region() const {
         return util::check(member_of)->region();
      }

      const ipr::Region& Bitfield::home_region() const {
         return util::check(member_of)->region();
      }
      ...
      const ipr::Type& Base_type::type() const { return base; }

      const ipr::Region& Base_type::lexical_region() const { return where; }

      const ipr::Region& Base_type::home_region() const { return where; }

      int Base_type::position() const { return scope_pos; }

      Optional<ipr::Expr> Base_type::initializer() const {
         throw std::domain_error("impl::Base_type::initializer");
      }

      const ipr::Sequence<ipr::Decl>& Base_type::decl_set() const {
         return overload.seq;
      }
      ...
      const ipr::Name& Enumerator::name() const { return id; }

      const ipr::Region& Enumerator::lexical_region() const {
         return *util::check(where);
      }

      const ipr::Region& Enumerator::home_region() const {
         return *util::check(where);
      }

      const ipr::Enum& Enumerator::membership() const { return constraint; }

      const ipr::Sequence<ipr::Decl>& Enumerator::decl_set() const {
         return overload.seq;
      }

      int Enumerator::position() const { return scope_pos; }

      Optional<ipr::Expr> Enumerator::initializer() const { return { init }; }

      // -----------------
      // -- impl::Field --
      // -----------------

      Field::Field() : member_of(0), init(0) {}

      Optional<ipr::Expr> Field::initializer() const { return { init }; }

      const ipr::Udt<ipr::Decl>& Field::membership() const {
         return *util::check(member_of);
      }

      const ipr::Region& Field::lexical_region() const {
         return util::check(member_of)->region();
      }

      const ipr::Region& Field::home_region() const {
         return util::check(member_of)->region();
      }
      ...
      Fundecl::Fundecl() : member_of(nullptr), data{}, lexreg(nullptr) {}

      const ipr::Parameter_list& Fundecl::parameters() const {
         if (data.index() == 0) return *util::check(data.parameters());
      ...
      const ipr::Udt<ipr::Decl>& Fundecl::membership() const {
         return *util::check(member_of);
      }
      ...
      const ipr::Region& Fundecl::lexical_region() const {
         return *util::check(lexreg);
      }
      ...
      const ipr::Named_map& Named_map::primary_named_map() const {
         return *util::check(util::check(decl_data.master_data)->primary);
      }

      const ipr::Sequence<ipr::Decl>& Named_map::specializations() const {
         return util::check(decl_data.master_data)->specs;
      }

      const ipr::Mapping& Named_map::mapping() const {
      ...
      const ipr::Region& Named_map::lexical_region() const {
         return *util::check(lexreg);
      }
      ...
      Parameter::Parameter(const ipr::Name& n, const impl::Rname& rn)
            : id(n), abstract_name(rn), where(0), init(0) {}

      const ipr::Name& Parameter::name() const { return id; }

      const ipr::Type& Parameter::type() const {
         return abstract_name.rep.first;
      }

      const ipr::Region& Parameter::home_region() const {
         return *util::check(where);
      }

      const ipr::Region& Parameter::lexical_region() const {
         return *util::check(where);
      }

      const ipr::Parameter_list& Parameter::membership() const {
         return *util::check(where);
      }

      const ipr::Sequence<ipr::Decl>& Parameter::decl_set() const {
         return overload.seq;
      }

      int Parameter::position() const { return abstract_name.rep.second; }

      Optional<ipr::Expr> Parameter::initializer() const { return { init }; }
      ...
      const ipr::Udt<ipr::Decl>& Typedecl::membership() const {
         return *util::check(member_of);
      }

      const ipr::Region& Typedecl::lexical_region() const {
         // return util::check(member_of)->region();
      ...
      Optional<ipr::Expr> Var::initializer() const { return { init }; }

      const ipr::Region& Var::lexical_region() const {
         return *util::check(lexreg);
      }
      ...
      Block::Block(const ipr::Region& pr, const ipr::Type& t) : region(&pr, t) {
         // >>>> pmp 16jun08
      ...
      const ipr::Type& Block::type() const { return region.scope.type(); }

      const ipr::Scope& Block::members() const { return region.scope; }

      const ipr::Sequence<ipr::Stmt>& Block::body() const { return stmt_seq; }

      const ipr::Sequence<ipr::Handler>& Block::handlers() const {
         return handler_seq;
      }
      ...
      const ipr::Type& For::type() const { return util::check(stmt)->type(); }

      const ipr::Expr& For::initializer() const { return *util::check(init); }

      const ipr::Expr& For::condition() const { return *util::check(cond); }

      const ipr::Expr& For::increment() const { return *util::check(inc); }

      const ipr::Stmt& For::body() const { return *util::check(stmt); }
      ...
      const ipr::Var& For_in::variable() const { return *util::check(var); }

      const ipr::Expr& For_in::sequence() const { return *util::check(seq); }

      const ipr::Type& For_in::type() const {
         return util::check(stmt)->type();
      }

      const ipr::Stmt& For_in::body() const { return *util::check(stmt); }
      ...
      const ipr::Type& Break::type() const { return util::check(stmt)->type(); }

      const ipr::Stmt& Break::from() const { return *util::check(stmt); }
      ...
      const ipr::Stmt& Continue::iteration() const {
      ...
      impl::Break* stmt_factory::make_break() { return breaks.make(); }

      impl::Continue* stmt_factory::make_continue() { return continues.make(); }

      impl::Empty_stmt* stmt_factory::make_empty_stmt() {
         return empty_stmts.make(*make_phantom());
      }

      impl::Block* stmt_factory::make_block(const ipr::Region& pr,
                                            const ipr::Type& t) {
         return blocks.make(pr, t);
      }

      impl::Ctor_body* stmt_factory::make_ctor_body(const ipr::Expr_list& m,
                                                    const ipr::Block& b) {
         return ctor_bodies.make(m, b);
      }

      impl::Expr_stmt* stmt_factory::make_expr_stmt(const ipr::Expr& e) {
         return expr_stmts.make(e);
      }

      impl::Goto* stmt_factory::make_goto(const ipr::Expr& e) {
         return gotos.make(e);
      }

      impl::Return* stmt_factory::make_return(const ipr::Expr& e) {
         return returns.make(e);
      }

      impl::Do* stmt_factory::make_do(const ipr::Stmt& s, const ipr::Expr& c) {
         return dos.make(c, s);
      }

      impl::If_then* stmt_factory::make_if_then(const ipr::Expr& c,
                                                const ipr::Stmt& s) {
         return ifs.make(c, s);
      }

      impl::Switch* stmt_factory::make_switch(const ipr::Expr& c,
                                              const ipr::Stmt& s) {
         return switches.make(c, s);
      }

      impl::Handler* stmt_factory::make_handler(const ipr::Decl& d,
                                                const ipr::Block& b) {
         return handlers.make(d, b);
      }

      impl::Labeled_stmt* stmt_factory::make_labeled_stmt(const ipr::Expr& l,
                                                          const ipr::Stmt& s) {
         return labeled_stmts.make(l, s);
      }

      impl::While* stmt_factory::make_while(const ipr::Expr& c,
                                            const ipr::Stmt& s) {
         return whiles.make(c, s);
      }

      impl::If_then_else* stmt_factory::make_if_then_else(const ipr::Expr& c,
                                                          const ipr::Stmt& t,
                                                          const ipr::Stmt& f) {
         return ifelses.make(c, t, f);
      }

      impl::For* stmt_factory::make_for() { return fors.make(); }

      impl::For_in* stmt_factory::make_for_in() { return for_ins.make(); }
      ...
      const ipr::Region& Enum::region() const { return body; }

      const ipr::Sequence<ipr::Enumerator>& Enum::members() const {
         return body.scope.decls.seq;
      }

      Enum::Kind Enum::kind() const { return enum_kind; }

      impl::Enumerator* Enum::add_member(const ipr::Name& n) {
      ...
      Class::Class(const ipr::Region& pr, const ipr::Type& t)
            : impl::Udt<ipr::Class>(&pr, t), base_subobjects(pr, t) {
         base_subobjects.owned_by = this;
      }

      const ipr::Sequence<ipr::Base_type>& Class::bases() const {
         return base_subobjects.scope.decls.seq;
      }

      impl::Base_type* Class::declare_base(const ipr::Type& t) {
      ...
      int String::size() const { return text.size(); }

      const char* String::begin() const { return text.begin(); }

      const char* String::end() const { return text.end(); }
      ...
      impl::Parameter* Parameter_list::add_member(const ipr::Name& n,
                                                  const impl::Rname& rn) {
      ...
      struct unary_compare {
         ...
         int operator()(const Unary<T>& lhs,
            ...
            return util::lexicographical_compare()(lhs.rep.begin(),
                                                   lhs.rep.end(), rhs.begin(),
                                                   rhs.end(), unary_compare());
         }
         ...
         int operator()(const ipr::Sequence<ipr::Type>& lhs,
            ...
            return util::lexicographical_compare()(
               lhs.begin(), lhs.end(), rhs.rep.begin(), rhs.rep.end(),
               unary_compare());
         }
         ...
         int operator()(const ref_sequence<T>& lhs,
            ...
            return util::lexicographical_compare()(
               lhs.begin(), lhs.end(), rhs.begin(), rhs.end(), unary_compare());
         }
      };


      inline bool operator==(const ipr::Node& lhs, const ipr::Node& rhs) {
      ...
      struct id_compare {
         int operator()(const ipr::Identifier& lhs,
                        const ipr::String& rhs) const {
            return compare(lhs.string(), rhs);
         }

         int operator()(const ipr::String& lhs,
                        const ipr::Identifier& rhs) const {
      ...
      // called with a type for which Pointer (Reference/sizeof etc.) is
      // created, on RHS we would be called with already allocated Pointer
      // types. Thus we have to check whether any of the existing Pointer types
      // does not already have a points_to (its operand()) equal to the type in
      // LHS.
      struct unified_type_compare {
         template<class Cat, class Operand>
         int operator()(const ipr::Type& lhs,
                        const ipr::Unary<Cat, Operand>& rhs) const {
            return compare(lhs, rhs.operand());
         }
      };
      // <<<< Yuriy Solodkyy: 2008/07/10

      impl::Array* type_factory::make_array(const ipr::Type& t,
                                            const ipr::Expr& b) {
         using rep = impl::Array::Rep;
         return arrays.insert(rep{ t, b }, binary_compare());
      }

      impl::Qualified* type_factory::make_qualified(ipr::Type_qualifier cv,
                                                    const ipr::Type& t) {
      ...
      impl::Decltype* type_factory::make_decltype(const ipr::Expr& e) {
      ...
      impl::Function* type_factory::make_function(const ipr::Product& s,
                                                  const ipr::Type& t,
                                                  const ipr::Sum& e,
                                                  const ipr::Linkage& l) {
      ...
      impl::Pointer* type_factory::make_pointer(const ipr::Type& t) {
         // >>>> Yuriy Solodkyy: 2008/07/10
      ...
      impl::Reference* type_factory::make_reference(const ipr::Type& t) {
      ...
      impl::Sum* type_factory::make_sum(const ipr::Sequence<ipr::Type>& seq) {
         return sums.insert(seq, unary_compare());
      }

      impl::Template* type_factory::make_template(const ipr::Product& s,
                                                  const ipr::Type& t) {
         using rep = impl::Template::Rep;
         return templates.insert(rep{ s, t }, binary_compare());
      }

      impl::Enum* type_factory::make_enum(const ipr::Region& pr,
                                          const ipr::Type& t, Enum::Kind k) {
      ...
      const ipr::Product& Expr_list::type() const { return seq; }

      const ipr::Sequence<ipr::Expr>& Expr_list::operand() const {
         return seq.seq;
      }
      ...
      const ipr::Type& Id_expr::type() const {
         return util::check(decl)->type();
      }

      const ipr::Decl& Id_expr::resolution() const {
      ...
      Mapping::Mapping(const ipr::Region& pr, const ipr::Type& t, int d)
            : parameters(pr, t), value_type(0), body(0), nesting_level(d) {
         // 31Oct08 added by PIR to avoid exceptions,
         //   when querying the region (parameters) for its owner
         parameters.owned_by = this;
      }

      const ipr::Parameter_list& Mapping::params() const { return parameters; }

      const ipr::Type& Mapping::result_type() const {
         return *util::check(value_type);
      }

      const ipr::Expr& Mapping::result() const { return *util::check(body); }

      int Mapping::depth() const { return nesting_level; }

      impl::Parameter* Mapping::param(const ipr::Name& n,
                                      const impl::Rname& rn) {
      ...
      const ipr::Sequence<ipr::Decl>& Scope::members() const {
         return decls.seq;
      }

      const ipr::Overload& Scope::operator[](const ipr::Name& n) const {
      ...
      template<class T>
      inline void Scope::add_member(T* decl) {
         decl->decl_data.scope_pos = decls.seq.size();
         decls.seq.insert(&decl->decl_data);
      }

      impl::Alias* Scope::make_alias(const ipr::Name& n, const ipr::Expr& i) {
      ...
      impl::Var* Scope::make_var(const ipr::Name& n, const ipr::Type& t) {
         impl::Overload* ovl    = overloads.insert(n, node_compare());
      ...
      impl::Field* Scope::make_field(const ipr::Name& n, const ipr::Type& t) {
         impl::Overload* ovl    = overloads.insert(n, node_compare());
         overload_entry* master = ovl->lookup(t);
      ...
      impl::Bitfield* Scope::make_bitfield(const ipr::Name& n,
                                           const ipr::Type& t) {
         impl::Overload* ovl    = overloads.insert(n, node_compare());
         overload_entry* master = ovl->lookup(t);
      ...
      impl::Typedecl* Scope::make_typedecl(const ipr::Name& n,
                                           const ipr::Type& t) {
      ...
      impl::Fundecl* Scope::make_fundecl(const ipr::Name& n,
                                         const ipr::Function& t) {
         impl::Overload* ovl    = overloads.insert(n, node_compare());
         overload_entry* master = ovl->lookup(t);
      ...
      impl::Named_map* Scope::make_primary_map(const ipr::Name& n,
      ...
      impl::Named_map* Scope::make_secondary_map(const ipr::Name& n,
                                                 const ipr::Template& t) {
         impl::Overload* ovl    = overloads.insert(n, node_compare());
         overload_entry* master = ovl->lookup(t);
      ...
      const ipr::Region& Region::enclosing() const {
         return *util::check(parent);
      }

      const ipr::Scope& Region::bindings() const { return scope; }

      const Region::location_span& Region::span() const { return extent; }

      const ipr::Expr& Region::owner() const { return *util::check(owned_by); }

      Region* Region::make_subregion() {
      ...
      const ipr::String& expr_factory::get_string(const char* s) {
         return get_string(s, std::strlen(s));
      }

      const ipr::String& expr_factory::get_string(const std::string& s) {
      ...
      struct string_comp {
         ...
         int operator()(const proxy& lhs, const impl::String& rhs) const {
            return util::lexicographical_compare()(
               lhs.first, lhs.first + lhs.second, rhs.begin(), rhs.end(),
               char_compare());
         }

         int operator()(const impl::String& lhs, const proxy& rhs) const {
            return util::lexicographical_compare()(
               lhs.begin(), lhs.end(), rhs.first, rhs.first + rhs.second,
               char_compare());
         }

         int operator()(const util::string& lhs,
                        const impl::String& rhs) const {
            return util::lexicographical_compare()(
               lhs.data, lhs.data + lhs.length, rhs.begin(), rhs.end(),
               char_compare());
         }

         int operator()(const impl::String& lhs,
                        const util::string& rhs) const {
            return util::lexicographical_compare()(
               lhs.begin(), lhs.end(), rhs.data, rhs.data + rhs.length,
               char_compare());
         }
      };

      const ipr::String& expr_factory::get_string(const char* s, int n) {
         const impl::String* item =
            strings.find(std::make_pair(s, n), string_comp());
         if (item == 0)
            item =
               strings.insert(*string_pool.make_string(s, n), string_comp());
      ...
      // -- Language linkage
      const ipr::Linkage& expr_factory::get_linkage(const char* s) {
         return get_linkage(get_string(s));
      }

      const ipr::Linkage& expr_factory::get_linkage(const std::string& s) {
         return get_linkage(get_string(s));
      }
      ...
      impl::Phantom* expr_factory::make_phantom() { return phantoms.make(); }

      const ipr::Phantom* expr_factory::make_phantom(const ipr::Type& t) {
         return phantoms.make(&t);
      }

      impl::Address* expr_factory::make_address(const ipr::Expr& e) {
         return addresses.make(e);
      }

      impl::Array_delete* expr_factory::make_array_delete(const ipr::Expr& e) {
         return array_deletes.make(e);
      }

      impl::Complement* expr_factory::make_complement(const ipr::Expr& e) {
         return complements.make(e);
      }

      impl::Conversion* expr_factory::make_conversion(const ipr::Type& t) {
         return convs.insert(t, unary_compare());
      }

      impl::Ctor_name* expr_factory::make_ctor_name(const ipr::Type& t) {
         return ctors.insert(t, unary_compare());
      }

      impl::Delete* expr_factory::make_delete(const ipr::Expr& e) {
         return deletes.make(e);
      }

      impl::Deref* expr_factory::make_deref(const ipr::Expr& e) {
         return derefs.make(e);
      }

      impl::Dtor_name* expr_factory::make_dtor_name(const ipr::Type& t) {
         return dtors.insert(t, unary_compare());
      }

      impl::Expr_list* expr_factory::make_expr_list() { return xlists.make(); }

      impl::Identifier* expr_factory::make_identifier(const ipr::String& s) {
         return ids.insert(s, id_compare());
      }

      impl::Identifier* expr_factory::make_identifier(const char* s) {
         return make_identifier(get_string(s));
      }

      impl::Identifier* expr_factory::make_identifier(const std::string& s) {
         return make_identifier(get_string(s));
      }


      impl::Id_expr* expr_factory::make_id_expr(const ipr::Name& n) {
         return id_exprs.make(n);
      }

      impl::Id_expr* expr_factory::make_id_expr(const ipr::Decl& d) {
      ...
      impl::Not* expr_factory::make_not(const ipr::Expr& e) {
         return nots.make(e);
      }

      impl::Operator* expr_factory::make_operator(const ipr::String& s) {
         return ops.insert(s, unary_compare());
      }

      impl::Operator* expr_factory::make_operator(const char* s) {
         return make_operator(get_string(s));
      }

      impl::Operator* expr_factory::make_operator(const std::string& s) {
         return make_operator(get_string(s));
      }

      impl::Paren_expr* expr_factory::make_paren_expr(const ipr::Expr& e) {
      ...
      impl::Throw* expr_factory::make_throw(const ipr::Expr& e) {
         return throws.make(e);
      }

      impl::Type_id* expr_factory::make_type_id(const ipr::Type& t) {
         return type_ids.insert(t, unary_compare());
      }

      impl::Sizeof* expr_factory::make_sizeof(const ipr::Expr& t) {
         return sizeofs.insert(t, unary_compare());
      }

      impl::Typeid* expr_factory::make_typeid(const ipr::Expr& t) {
         return xtypeids.insert(t, unary_compare());
      }

      impl::Unary_minus* expr_factory::make_unary_minus(const ipr::Expr& e) {
         return unary_minuses.make(e);
      }

      impl::Unary_plus* expr_factory::make_unary_plus(const ipr::Expr& e) {
         return unary_pluses.make(e);
      }

      impl::Expansion* expr_factory::make_expansion(const ipr::Expr& e) {
         return expansions.make(e);
      }

      impl::And* expr_factory::make_and(const ipr::Expr& l,
                                        const ipr::Expr& r) {
         return ands.make(l, r);
      }

      impl::Array_ref* expr_factory::make_array_ref(const ipr::Expr& l,
                                                    const ipr::Expr& r) {
         return array_refs.make(l, r);
      }

      impl::Arrow* expr_factory::make_arrow(const ipr::Expr& l,
                                            const ipr::Expr& r) {
         return arrows.make(l, r);
      }

      impl::Arrow_star* expr_factory::make_arrow_star(const ipr::Expr& l,
                                                      const ipr::Expr& r) {
         return arrow_stars.make(l, r);
      }

      impl::Assign* expr_factory::make_assign(const ipr::Expr& l,
                                              const ipr::Expr& r) {
         return assigns.make(l, r);
      }

      impl::Bitand* expr_factory::make_bitand(const ipr::Expr& l,
                                              const ipr::Expr& r) {
      ...
      impl::Bitor* expr_factory::make_bitor(const ipr::Expr& l,
                                            const ipr::Expr& r) {
         return bitors.make(l, r);
      }

      impl::Bitor_assign* expr_factory::make_bitor_assign(const ipr::Expr& l,
                                                          const ipr::Expr& r) {
         return bitor_assigns.make(l, r);
      }

      impl::Bitxor* expr_factory::make_bitxor(const ipr::Expr& l,
                                              const ipr::Expr& r) {
      ...
      impl::Cast* expr_factory::make_cast(const ipr::Type& t,
                                          const ipr::Expr& e) {
         return casts.make(t, e);
      }

      impl::Call* expr_factory::make_call(const ipr::Expr& l,
                                          const ipr::Expr_list& r) {
         return calls.make(l, r);
      }

      impl::Comma* expr_factory::make_comma(const ipr::Expr& l,
                                            const ipr::Expr& r) {
         return commas.make(l, r);
      }

      impl::Const_cast* expr_factory::make_const_cast(const ipr::Type& t,
                                                      const ipr::Expr& e) {
         return ccasts.make(t, e);
      }

      impl::Datum* expr_factory::make_datum(const ipr::Type& t,
                                            const ipr::Expr_list& e) {
         return data.make(t, e);
      }

      impl::Div* expr_factory::make_div(const ipr::Expr& l,
                                        const ipr::Expr& r) {
         return divs.make(l, r);
      }

      impl::Div_assign* expr_factory::make_div_assign(const ipr::Expr& l,
                                                      const ipr::Expr& r) {
         return div_assigns.make(l, r);
      }

      impl::Dot* expr_factory::make_dot(const ipr::Expr& l,
                                        const ipr::Expr& r) {
         return dots.make(l, r);
      }

      impl::Dot_star* expr_factory::make_dot_star(const ipr::Expr& l,
                                                  const ipr::Expr& r) {
         return dot_stars.make(l, r);
      }

      impl::Dynamic_cast* expr_factory::make_dynamic_cast(const ipr::Type& t,
                                                          const ipr::Expr& e) {
         return dcasts.make(t, e);
      }

      impl::Equal* expr_factory::make_equal(const ipr::Expr& l,
                                            const ipr::Expr& r) {
         return equals.make(l, r);
      }

      impl::Greater* expr_factory::make_greater(const ipr::Expr& l,
                                                const ipr::Expr& r) {
      ...
      impl::Less* expr_factory::make_less(const ipr::Expr& l,
                                          const ipr::Expr& r) {
         return lesses.make(l, r);
      }

      impl::Less_equal* expr_factory::make_less_equal(const ipr::Expr& l,
                                                      const ipr::Expr& r) {
         return less_equals.make(l, r);
      }

      impl::Literal* expr_factory::make_literal(const ipr::Type& t,
                                                const ipr::String& s) {
      ...
      impl::Lshift* expr_factory::make_lshift(const ipr::Expr& l,
                                              const ipr::Expr& r) {
      ...
      impl::Member_init* expr_factory::make_member_init(const ipr::Expr& l,
                                                        const ipr::Expr& r) {
         return member_inits.make(l, r);
      }

      impl::Minus* expr_factory::make_minus(const ipr::Expr& l,
                                            const ipr::Expr& r) {
         return minuses.make(l, r);
      }

      impl::Minus_assign* expr_factory::make_minus_assign(const ipr::Expr& l,
                                                          const ipr::Expr& r) {
         return minus_assigns.make(l, r);
      }

      impl::Modulo* expr_factory::make_modulo(const ipr::Expr& l,
                                              const ipr::Expr& r) {
      ...
      impl::Mul* expr_factory::make_mul(const ipr::Expr& l,
                                        const ipr::Expr& r) {
         return muls.make(l, r);
      }

      impl::Mul_assign* expr_factory::make_mul_assign(const ipr::Expr& l,
                                                      const ipr::Expr& r) {
         return mul_assigns.make(l, r);
      }

      impl::Not_equal* expr_factory::make_not_equal(const ipr::Expr& l,
                                                    const ipr::Expr& r) {
         return not_equals.make(l, r);
      }

      impl::Or* expr_factory::make_or(const ipr::Expr& l, const ipr::Expr& r) {
         return ors.make(l, r);
      }

      impl::Plus* expr_factory::make_plus(const ipr::Expr& l,
                                          const ipr::Expr& r) {
         return pluses.make(l, r);
      }

      impl::Plus_assign* expr_factory::make_plus_assign(const ipr::Expr& l,
                                                        const ipr::Expr& r) {
      ...
      impl::Scope_ref* expr_factory::make_scope_ref(const ipr::Expr& l,
                                                    const ipr::Expr& r) {
         using Rep = impl::Scope_ref::Rep;
         return scope_refs.insert(Rep{ l, r }, binary_compare());
      }

      impl::Rshift* expr_factory::make_rshift(const ipr::Expr& l,
                                              const ipr::Expr& r) {
      ...
      impl::Static_cast* expr_factory::make_static_cast(const ipr::Type& t,
                                                        const ipr::Expr& e) {
         return scasts.make(t, e);
      }

      impl::New* expr_factory::make_new(const ipr::Type& t) {
      ...
      impl::New* expr_factory::make_new(const ipr::Expr_list& p,
                                        const ipr::Type& t,
                                        const ipr::Expr_list& i) {
         return news.make(&p, t, &i);
      }

      impl::Conditional* expr_factory::make_conditional(const ipr::Expr& c,
                                                        const ipr::Expr& t,
                                                        const ipr::Expr& f) {
         return conds.make(c, t, f);
      }

      impl::Rname* expr_factory::rname_for_next_param(const impl::Mapping& map,
                                                      const ipr::Type& t) {
         using Rep = impl::Rname::Rep;
         return rnames.insert(
            Rep{ t, map.nesting_level, map.parameters.size() },
            ternary_compare());
      }

      impl::Mapping* expr_factory::make_mapping(const ipr::Region& r,
                                                const ipr::Type& t, int l) {
      ...
      Lexicon::Lexicon()
            : anytype(get_identifier("typename"), cxx_linkage(), anytype),
              ...
              namespacetype(get_identifier("namespace"), cxx_linkage(),
                            anytype),
              ...
              ulongtype(get_identifier("unsigned long"), cxx_linkage(),
                        anytype),
              longlongtype(get_identifier("long long"), cxx_linkage(), anytype),
              ulonglongtype(get_identifier("unsigned long long"), cxx_linkage(),
                            anytype),
              floattype(get_identifier("float"), cxx_linkage(), anytype),
              doubletype(get_identifier("double"), cxx_linkage(), anytype),
              longdoubletype(get_identifier("long double"), cxx_linkage(),
                             anytype),
      ...
      const ipr::Literal& Lexicon::get_literal(const ipr::Type& t,
                                               const char* s) {
      ...
      const ipr::Identifier& Lexicon::get_identifier(const char* s) {
         return get_identifier(get_string(s));
      }

      const ipr::Identifier& Lexicon::get_identifier(const std::string& s) {
         return get_identifier(get_string(s));
      }

      const ipr::Identifier& Lexicon::get_identifier(const ipr::String& s) {
         impl::Identifier* id = expr_factory::make_identifier(s);
         if (id->constraint == 0) id->constraint = &get_decltype(*id);
         return *id;
      }
      ...
      const ipr::Type& Lexicon::ulong_long_type() const {
      ...
      T* Lexicon::finish_type(T* t) {
         if (t->constraint == 0) t->constraint = &anytype;

         if (t->id == 0) t->id = make_type_id(*t);

         return t;
      }

      const ipr::Ctor_name& Lexicon::get_ctor_name(const ipr::Type& t) {
         impl::Ctor_name* id = expr_factory::make_ctor_name(t);
         if (id->constraint == 0) id->constraint = &get_decltype(*id);
         return *id;
      }

      const ipr::Dtor_name& Lexicon::get_dtor_name(const ipr::Type& t) {
         impl::Dtor_name* id = expr_factory::make_dtor_name(t);
         if (id->constraint == 0) id->constraint = &get_decltype(*id);
         return *id;
      }

      const ipr::Operator& Lexicon::get_operator(const char* s) {
         return get_operator(get_string(s));
      }

      const ipr::Operator& Lexicon::get_operator(const std::string& s) {
         return get_operator(get_string(s));
      }

      const ipr::Operator& Lexicon::get_operator(const ipr::String& s) {
         impl::Operator* op = expr_factory::make_operator(s);
         if (op->constraint == 0) op->constraint = &get_decltype(*op);
         return *op;
      }

      const ipr::Conversion& Lexicon::get_conversion(const ipr::Type& t) {
         impl::Conversion* conv = expr_factory::make_conversion(t);
         if (conv->constraint == 0) conv->constraint = &get_decltype(*conv);
         return *conv;
      }

      const ipr::Scope_ref& Lexicon::get_scope_ref(const ipr::Expr& s,
                                                   const ipr::Expr& m) {
         impl::Scope_ref* sr = expr_factory::make_scope_ref(s, m);
         if (sr->constraint == 0) sr->constraint = &get_decltype(*sr);
         return *sr;
      }
      ...
      Lexicon::get_template_id(const ipr::Name& t, const ipr::Expr_list& a) {
         ...
         if (tid->constraint == 0) tid->constraint = &get_decltype(*tid);
         return *tid;
      }

      const ipr::Array& Lexicon::get_array(const ipr::Type& t,
                                           const ipr::Expr& b) {
         return *finish_type(types.make_array(t, b));
      }

      const ipr::As_type& Lexicon::get_as_type(const ipr::Expr& e) {
      ...
      const ipr::Decltype& Lexicon::get_decltype(const ipr::Expr& e) {
         return *finish_type(types.make_decltype(e));
      }

      const ipr::Function& Lexicon::get_function(const ipr::Product& p,
                                                 const ipr::Type& t,
                                                 const ipr::Sum& s,
                                                 const ipr::Linkage& l) {
      ...
      const ipr::Pointer& Lexicon::get_pointer(const ipr::Type& t) {
      ...
      const ipr::Ptr_to_member& Lexicon::get_ptr_to_member(const ipr::Type& s,
                                                           const ipr::Type& t) {
         return *finish_type(types.make_ptr_to_member(s, t));
      }

      const ipr::Qualified& Lexicon::get_qualified(ipr::Type_qualifier cv,
                                                   const ipr::Type& t) {
         assert(cv != ipr::Type_qualifier::None);
         return *finish_type(types.make_qualified(cv, t));
      }

      const ipr::Reference& Lexicon::get_reference(const ipr::Type& t) {
      ...
      const ipr::Sum& Lexicon::get_sum(const ref_sequence<ipr::Type>& s) {
         return *finish_type(
            types.make_sum(*type_seqs.insert(s, unary_compare())));
      }

      const ipr::Template& Lexicon::get_template(const ipr::Product& p,
                                                 const ipr::Type& t) {
         return *finish_type(types.make_template(p, t));
      }

      impl::Class* Lexicon::make_class(const ipr::Region& pr) {
         return types.make_class(pr, classtype);
      }

      impl::Enum* Lexicon::make_enum(const ipr::Region& pr, Enum::Kind k) {
         return types.make_enum(pr, enumtype, k);
      }

      impl::Namespace* Lexicon::make_namespace(const ipr::Region& pr) {
         return types.make_namespace(&pr, namespacetype);
      }

      impl::Union* Lexicon::make_union(const ipr::Region& pr) {
         return types.make_union(pr, uniontype);
      }

      impl::Mapping* Lexicon::make_mapping(const ipr::Region& r) {
         return expr_factory::make_mapping(r, anytype);
      }

      impl::Parameter* Lexicon::make_parameter(const ipr::Name& n,
                                               const ipr::Type& t,
                                               impl::Mapping& m) {
      ...
      Module::implementation_units() const {
         return units;
      }
...
int main()
{
   ...
   // then its type,
   auto& type = lexicon.get_qualified(Type_qualifier::Const,
lexicon.int_type());
