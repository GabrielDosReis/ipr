...
namespace ipr {
   ...
   inline bool structurally_same(const Unary<Cat, Op>& lhs,
                                 const Unary<Cat, Op>& rhs) {
      return physically_same(lhs, rhs)
             || structurally_same(lhs.operand(), rhs.operand());
   }

   template<class Cat, class Op1, class Op2>
   inline bool structurally_same(const Binary<Cat, Op1, Op2>& lhs,
   ...
   inline bool structurally_same(const Ternary<Cat, Op1, Op2, Op3>& lhs,
...
void ipr::Missing_overrider::operator()(const ipr::Node& n) const {
   throw std::logic_error(std::string("missing overrider for ")
                          + typeid(n).name());
}

// -- ipr::Visitor --
void ipr::Visitor::visit(const Annotation&) {}
...
void ipr::Visitor::visit(const Linkage& l) { visit(as<Node>(l)); }
...
void ipr::Visitor::visit(const Name& n) { visit(as<Expr>(n)); }
...
void ipr::Visitor::visit(const Scope_ref& n) { visit(as<Name>(n)); }
...
void ipr::Visitor::visit(const Rname& n) { visit(as<Name>(n)); }
...
void ipr::Visitor::visit(const Template& t) { visit(as<Type>(t)); }
...
void ipr::Visitor::visit(const Initializer_list& e) { visit(as<Expr>(e)); }
...
void ipr::Visitor::visit(const Paren_expr& e) { visit(as<Classic>(e)); }
...
void ipr::Visitor::visit(const Datum& e) { visit(as<Classic>(e)); }
...
void ipr::Visitor::visit(const If_then& s) { visit(as<Stmt>(s)); }

void ipr::Visitor::visit(const If_then_else& s) { visit(as<Stmt>(s)); }
...
void ipr::Visitor::visit(const Asm& d) { visit(as<Decl>(d)); }
...
void ipr::Visitor::visit(const Named_map& d) { visit(as<Decl>(d)); }
...
//
void ipr::Visitor::visit(const Global_scope& s) { visit(as<Namespace>(s)); }

//
// Similar observation holds for the null-statement.
//
void ipr::Visitor::visit(const Empty_stmt& s) { visit(as<Expr_stmt>(s)); }

void ipr::Translation_unit::Visitor::visit(const Module_unit& u) {
   visit(as<Translation_unit>(u));
}

void ipr::Translation_unit::Visitor::visit(const Interface_unit& u) {
   visit(as<Translation_unit>(u));
}
