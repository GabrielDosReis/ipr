...
namespace ipr {
   ...
   Printer::Printer(std::ostream& os)
         : stream(os), pad(None), emit_newline(false), pending_indentation(0) {}
   ...
   void Printer::write(const char* begin, const char* last) {
   ...
   template<typename T>
   inline Token_helper<T> token(T t) {
      return Token_helper<T>(t);
   }

   template<typename T>
   inline Printer& operator<<(Printer& printer, Token_helper<T> t) {
      return printer << t.value << Printer::None;
   }

   inline Printer& insert_xtoken(Printer& printer, const char* s) {
      return printer << s << Printer::None;
   }

   struct needs_newline {};

   inline Printer& operator<<(Printer& printer, needs_newline) {
   ...
   Printer& operator<<(Printer& printer, newline) {
      printer << token('\n');
      const int n = printer.indent();
      for (int i = 0; i < n; ++i) printer << ' ';
      printer.needs_newline(false);
      return printer;
   }

   // -- An Expr_list is mostly a expression-seq.  See print_sequence().
   static inline Printer& operator<<(Printer& pp, const Expr_list& l) {
      const int n = l.size();
      for (int i = 0; i < n; ++i) {
         if (i != 0) pp << token(", ");
         pp << xpr_expr(l[i]);
   ...
   static inline Printer& operator<<(Printer& pp, const Sequence<Type>& s) {
      const int n = s.size();
      for (int i = 0; i < n; ++i) {
         if (i != 0) pp << token(", ");
         pp << xpr_type(s[i]);
   ...
   static inline Printer& operator<<(Printer& pp, const Parameter_list& l) {
      const int n = l.size();
      for (int i = 0; i < n; ++i) {
         if (i != 0) pp << token(", ");
         pp << xpr_decl(l[i]);
   ...
   inline Printer& operator<<(Printer& printer, indentation i) {
   ...
   inline Printer& operator<<(Printer& printer, newline_and_indent m) {
   ...
   struct xpr_identifier {
      ...
      template<int N>
      explicit xpr_identifier(const char (&s)[N]) : begin(s), last(s + N - 1) {}
   };

   static inline Printer& operator<<(Printer& printer, xpr_identifier id) {
      if (printer.padding() == Printer::Before) printer << ' ';
      printer.write(id.begin, id.last);

      return printer << Printer::Before;
   }

   Printer& operator<<(Printer& pp, const Identifier& id) {
   ...
   namespace xpr {
      struct Name : pp_base {
         ...
         void visit(const Identifier& id) override { pp << id; }
         ...
         void visit(const Conversion& c) override {
            // For now only regular cast, later we'll add support for
            // overloading dynamic_cast, reinterpret_cast, const_cast and
            // static_cast
            pp << xpr_identifier("operator") << xpr_identifier("cast")
         ...
         void visit(const Template_id& n) override {
            pp << xpr_primary_expr(n.template_name()) << token("<|") << n.args()
               << token("|>");
         }
         ...
         void visit(const Rname& rn) override {
            pp << xpr_identifier("#(") << rn.level() << token(", ")
               << rn.position() << token(')');
         }
   ...
   static inline Printer& operator<<(Printer& printer, xpr_name x) {
   ...
   namespace xpr {
      struct Primary_expr : xpr::Name {
         ...
         void visit(const Id_expr& id) override {
            pp << xpr_name(id.name(), id.resolution());
         }
      ...
      void Primary_expr::visit(const ipr::Literal& l) {
   ...
   static inline Printer& operator<<(Printer& printer, xpr_primary_expr x) {
   ...
   void new_style_cast(Printer& pp, const Cast_expr<code>& e,
                       const char (&op)[N]) {
      pp << xpr_identifier(op) << token("<|") << xpr_type(e.type())
         << token("|>") << token('(') << xpr_expr(e.expr()) << token(')');
   }
   ...
   namespace xpr {
      ...
      struct Postfix_expr : xpr::Primary_expr {
         ...
         void visit(const Call& e) override {
            pp << xpr_postfix_expr(e.function()) << token('(') << e.args()
               << token(')');
         }

         void visit(const Datum& e) override {
            pp << xpr_type(e.type()) << token('(') << e.args() << token(')');
         }
         ...
         void visit(const Typeid& e) override {
            pp << xpr_identifier("typeid") << token('(')
               << xpr_expr(e.operand()) << token(')');
         }
      };
   } // namespace xpr


   static inline Printer& operator<<(Printer& printer, xpr_postfix_expr x) {
   ...
   template<class T>
   inline void unary_operation(Printer& pp, const Unary<T>& e, const char* op) {
   ...
   namespace xpr {
      struct Unary_expr : xpr::Postfix_expr {
         ...
         void visit(const Pre_decrement& e) override {
            unary_operation(pp, e, "--");
         }

         void visit(const Pre_increment& e) override {
            unary_operation(pp, e, "++");
         }

         void visit(const Address& e) override { unary_operation(pp, e, "&"); }

         void visit(const Complement& e) override {
            unary_operation(pp, e, "~");
         }

         void visit(const Deref& e) override { unary_operation(pp, e, "*"); }

         void visit(const Unary_minus& e) override {
            unary_operation(pp, e, "-");
         }
         ...
         void visit(const Delete& e) override {
            pp << xpr_identifier("delete") << token(' ')
   ...
   namespace xpr {
      struct Cast_expr : xpr::Unary_expr {
         ...
         void visit(const Cast& e) override { new_style_cast(pp, e, "cast"); }
      };
   } // namespace xpr

   static inline Printer& operator<<(Printer& printer, xpr_cast_expr x) {
   ...
   static void offset_with_pm(Printer& pp, const Member_selection<code>& e,
                              const char* op) {
      pp << xpr_pm_expr(e.base()) << op << xpr_cast_expr(e.member());
   }
   ...
   namespace xpr {
      struct Pm_expr : xpr::Cast_expr {
         ...
         void visit(const Arrow_star& e) override {
            offset_with_pm(pp, e, "->*");
         }
      };
   } // namespace xpr

   static inline Printer& operator<<(Printer& printer, xpr_pm_expr x) {
   ...
   inline void binary_expression(Printer& pp, const E& e, Op op) {
      pp << Left(e.first()) << token(' ') << op << token(' ')
         << Right(e.second());
   }
   ...
   static inline Printer& operator<<(Printer& printer, xpr_mul_expr x) {
   ...
   static inline Printer& operator<<(Printer& printer, xpr_add_expr x) {
   ...
   static inline Printer& operator<<(Printer& printer, xpr_shift_expr x) {
   ...
   static inline Printer& operator<<(Printer& printer, xpr_rel_expr x) {
   ...
   static inline Printer& operator<<(Printer& printer, xpr_eq_expr x) {
   ...
   static inline Printer& operator<<(Printer& printer, xpr_and_expr x) {
   ...
   static inline Printer& operator<<(Printer& printer, xpr_xor_expr x) {
   ...
   static inline Printer& operator<<(Printer& printer, xpr_ior_expr x) {
   ...
   static inline Printer& operator<<(Printer& printer, xpr_land_expr x) {
   ...
   static inline Printer& operator<<(Printer& printer, xpr_lor_expr x) {
   ...
   namespace xpr {
      struct Cond_expr : xpr::Lor_expr {
         ...
         void visit(const Conditional& e) override {
            pp << xpr_lor_expr(e.condition()) << token(" ? ")
               << xpr_expr(e.then_expr()) << token(" : ")
   ...
   struct xpr_mapping_expression_visitor : pp_base {
      ...
      xpr_mapping_expression_visitor(Printer& p, const ipr::Mapping& m)
            : pp_base(p), map(m) {}
   ...
   Printer& operator<<(Printer& pp, xpr_mapping_expression e) {
   ...
   namespace xpr {
      struct Assignment_expr : xpr::Cond_expr {
         ...
         void visit(const Assign& e) override {
            binary_expression<xpr_lor_expr, xpr_assignment_expression>(pp, e,
                                                                       '=');
         }
   ...
   Printer& operator<<(Printer& printer, xpr_assignment_expression x) {
   ...
   struct xpr_expr_visitor : pp_base {
      ...
      void visit(const Scope& s) override {
         ...
         for (int i = 0; i < n; ++i) {
            pp << xpr_decl(decls[i], true) << newline();
         }
      }
      ...
      void visit(const Member_init& e) override {
         pp << xpr_expr(e.member()) << token('(') << xpr_expr(e.initializer())
            << token(')');
      }
   ...
   Printer& operator<<(Printer& printer, xpr_expr x) {
   ...
   static Printer& operator<<(Printer& printer, xpr_exception_spec x) {
      return printer << token(' ') << xpr_identifier("throw") << token('(')
                     << xpr_type(x.type) << token(')');
   }
   ...
   static Printer& operator<<(Printer& pp, xpr_base_classes x) {
   ...
   Printer& operator<<(Printer& printer, Type_qualifier cv) {
   ...
   static Printer& operator<<(Printer& pp, const ipr::Udt<T>& t) {
      return pp << token(' ') << token('{') << newline_and_indent(3)
                << xpr_expr(t.scope()) << newline_and_indent(-3) << token('}')
                << needs_newline();
   }
   ...
   // Therefore this class was moved from being a local class of subsequent
   // function to being just a regular class, which that function uses.
   static Printer& operator<<(Printer& printer, xpr_type_expr x);
   ...
   struct xpr_type_expr_visitor : pp_base {
      ...
      void visit(const As_type& t) override { pp << xpr_expr(t.expr()); }

      void visit(const Class& c) override {
         pp << xpr_base_classes(c.bases()) << token(' ') << token('{')
            << newline_and_indent(3) << xpr_expr(c.scope())
            << newline_and_indent(-3) << token('}') << needs_newline();
      }

      void visit(const Decltype& t) override {
         pp << xpr_identifier("decltype") << token(' ') << token('(')
            << xpr_expr(t.expr()) << token(')');
      }

      void visit(const Function& f) override {
         pp << token('(') << f.source().operand() << token(')')
            << xpr_exception_spec(f.throws()) << xpr_type(f.target());
      }
      ...
      void visit(const Ptr_to_member& t) override {
         pp << token('*') << token('[') << xpr_type(t.containing_type())
            << token(']') << token(',') << xpr_type(t.member_type());
      }

      void visit(const Qualified& t) override {
         pp << t.qualifiers() << xpr_type(t.main_variant());
      }
      ...
      void visit(const Template& t) override {
         pp << token('<') << t.source().operand() << token('>') << token(' ')
   ...
   struct xpr_type_visitor : pp_base {
      ...
      void visit(const Array& a) override { pp << xpr_type_expr(a); }

      void visit(const Function& f) override { pp << xpr_type_expr(f); }

      void visit(const Pointer& t) override { pp << xpr_type_expr(t); }

      void visit(const Ptr_to_member& t) override { pp << xpr_type_expr(t); }

      void visit(const Qualified& t) override { pp << xpr_type_expr(t); }

      void visit(const Reference& t) override { pp << xpr_type_expr(t); }

      void visit(const Rvalue_reference& t) override { pp << xpr_type_expr(t); }

      void visit(const Template& t) override { pp << xpr_type_expr(t); }
      ...
      void visit(const Product& t) override { pp << t.operand(); }

      void visit(const Sum& t) override { pp << t.operand(); }
   };
   // <<<< Yuriy Solodkyy: 2006/05/31

   Printer& operator<<(Printer& printer, xpr_type x) {
   ...
   static Printer& operator<<(Printer& printer, xpr_initializer x) {
      struct V : xpr::Assignment_expr {
         V(Printer& p) : xpr::Assignment_expr(p) {}

         void visit(const ipr::Type& t) override { pp << xpr_type_expr(t); }

         void visit(const ipr::Expr& e) override { pp << xpr_expr(e); }

         void visit(const ipr::Stmt& e) override { pp << xpr_stmt(e); }

         void visit(const ipr::Decl& e) override { pp << xpr_decl(e); }
      };
      V pp(printer);
      x.expr.accept(pp);
      return printer;
   ...
   namespace xpr {
      struct Stmt : xpr::Assignment_expr {
         ...
         void visit(const Expr_stmt& e) override {
            pp << xpr_expr(e.expr()) << token(';') << needs_newline();
         }
         ...
         void visit(const Labeled_stmt& s) override {
            ...
            pp << xpr_identifier("label") << token(' ') << xpr_expr(s.label())
               << token(':') << indentation(3) << needs_newline()
               << xpr_stmt(s.stmt()) << needs_newline();
         }

         void visit(const Block& s) override {
            pp << token('{') << needs_newline() << indentation(3);
            const Sequence<ipr::Stmt>& body = s.body();
            int n                           = body.size();
            for (int i = 0; i < n; ++i)
               pp << xpr_stmt(body[i]) << needs_newline();
            pp << newline_and_indent(-3) << token('}') << needs_newline();

            const Sequence<Handler>& handlers = s.handlers();
            n                                 = handlers.size();
            for (int i = 0; i < n; ++i) pp << xpr_stmt(handlers[i], false);
         }

         void visit(const Ctor_body& b) override {
            const Expr_list& inits = b.inits();
            if (inits.size() > 0)
               pp << token(" : ") << inits << needs_newline();

            pp << needs_newline() << xpr_stmt(b.block());
         }

         void visit(const If_then& s) override {
            pp << xpr_identifier("if") << token(' ') << token('(')
               << xpr_expr(s.condition()) << token(')') << newline_and_indent(3)
               << xpr_stmt(s.then_stmt(), false) << newline_and_indent(-3)
               << xpr_identifier("else") << newline_and_indent(3)
         ...
         void visit(const If_then_else& s) override {
            pp << xpr_identifier("if") << token(' ') << token('(')
               << xpr_expr(s.condition()) << token(')') << newline_and_indent(3)
               << xpr_stmt(s.then_stmt()) << newline_and_indent(-3)
               << xpr_identifier("else") << newline_and_indent(3)
               << xpr_stmt(s.else_stmt()) << indentation(-3) << needs_newline();
         }

         void visit(const Return& s) override {
            pp << xpr_identifier("return") << token(' ') << xpr_expr(s.value())
               << token(';') << needs_newline();
         }

         void visit(const Switch& s) override {
            pp << xpr_identifier("switch") << token(' ') << token('(')
               << xpr_expr(s.condition()) << token(')') << newline_and_indent(3)
               << xpr_stmt(s.body()) << newline_and_indent(-3);
         }

         void visit(const While& s) override {
            pp << xpr_identifier("while") << token(' ') << token('(')
               << xpr_expr(s.condition()) << token(')') << newline_and_indent(3)
               << xpr_stmt(s.body()) << needs_newline() << indentation(-3);
         }

         void visit(const Do& s) override {
            pp << xpr_identifier("do") << newline_and_indent(3)
               << xpr_stmt(s.body()) << newline_and_indent(-3)
               << xpr_identifier("while") << token(' ') << token('(')
               << xpr_expr(s.condition()) << token(')') << token(';')
               << needs_newline();
         }

         void visit(const For& s) override {
            pp << xpr_identifier("for") << token(" (")
               << xpr_expr(s.initializer()) << token("; ")
               << xpr_expr(s.condition()) << token("; ")
               << xpr_expr(s.increment()) << token(')') << newline_and_indent(3)
               << xpr_stmt(s.body()) << indentation(-3) << needs_newline();
         }

         void visit(const For_in& s) override {
            pp << xpr_identifier("for") << token(" (") << xpr_decl(s.variable())
               << token(" <- ") << xpr_expr(s.sequence()) << token(')')
               << newline_and_indent(3) << xpr_stmt(s.body()) << indentation(-3)
               << needs_newline();
         }

         void visit(const Break&) override {
            pp << xpr_identifier("break") << token(';') << needs_newline();
         }

         void visit(const Continue&) override {
            pp << xpr_identifier("continue") << token(';') << needs_newline();
         }

         void visit(const Goto& s) override {
            pp << xpr_identifier("goto") << token(' ') << xpr_expr(s.target())
               << token(';') << needs_newline();
         }

         void visit(const Handler& s) override {
            pp << xpr_identifier("catch") << token(' ') << token('(')
               << xpr_decl(s.exception()) << token(')') << newline_and_indent(3)
               << xpr_stmt(s.body()) << newline_and_indent(-3);
         }
   ...
   Printer& operator<<(Printer& printer, xpr_stmt x) {
      if (printer.needs_newline()) printer << newline_and_indent();
   ...
   Printer& operator<<(Printer& printer, DeclSpecifiers spec) {
   ...
   namespace xpr {
      struct Decl : xpr::Stmt {
         ...
         void visit(const ipr::Alias& d) override {
            pp << ipr::xpr_name(d) << token(" : ") << d.specifiers()
               << token(" typedef ") << xpr_expr(d.initializer().get());
         }
         ...
         void visit(const Typedecl& d) override {
            pp << ipr::xpr_name(d) << token(" : ") << xpr_type(d.type());

            if (auto init = d.initializer()) pp << xpr_type_expr(init.get());
         }
         ...
         void visit(const Bitfield& b) override {
            b.name().accept(*this);
            pp << token(" : #") << xpr_identifier("bitfield") << token('(')
               << xpr_expr(b.precision()) << token(')') << xpr_type(b.type());
         }
         ...
         void visit(const Fundecl& f) override {
            pp << ipr::xpr_name(f) << token(" : ") << f.specifiers() << ' '
               << token('(') << f.parameters() << token(')');

            const Function* pfn = util::view<Function>(f.type());
            if (pfn) pp << xpr_type(pfn->target()); // dump result type

            if (auto init = f.initializer())
               pp << needs_newline() << xpr_stmt(init.get());
         }

         void visit(const Named_map& m) override {
            m.name().accept(*this);
            pp << token(" : ") << xpr_mapping_expression(m.mapping());
         }
      };
   } // namespace xpr

   Printer& operator<<(Printer& printer, xpr_decl x) {
      if (printer.needs_newline()) printer << newline_and_indent();

      xpr::Decl impl(printer);
      x.decl.accept(impl);
      if (x.needs_semicolon) printer << token(';');
      return printer;
   }

   Printer& operator<<(Printer& pp, const Translation_unit& unit) {
