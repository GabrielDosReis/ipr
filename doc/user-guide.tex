...
%% types in IPR: they characterize type expresions such as class-expressions,
...
Template specialization names, \NodeClass{Specialization} in IPR, are made out
of two expressions, as:
\begin{Program}
  \expr{unit}.make_specialization(\expr{template}, \expr{arglist});
\end{Program}
...
class for a operator has a field \code{op\_impl} to record the function
...
\code{impl::Var}.  Manifacturing a node of that type requires ---
...
distinct node (\code{Field}).  However, the steps for manifacturing such node
...
all have types uniformely represented by a \code{Function} node.
